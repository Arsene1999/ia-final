...
import aima.core.search.csp.examples.Resposta;
...
public class Assignment<VAR extends Variable, VAL> implements Cloneable {
    ...
    public ArrayList<Resposta> RetornaOMapa(){
    	ArrayList<Resposta> res = new ArrayList<>();
    	for (Map.Entry<VAR, VAL> entry : variableToValueMap.entrySet()) {
    		//System.out.println(entry.getKey() +"  "+ entry.getValue());
    		horaDia aux = (horaDia) entry.getValue();
    		res.add(new Resposta(entry.getKey().getName(), aux.getDia(),aux.getHorario()));
    	}
    	return res;
    }
}